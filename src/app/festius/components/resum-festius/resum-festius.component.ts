...
export class ResumFestiusComponent {
  ...
  public llista_festius$: Observable<PeriodeFestius[]> = new Observable<PeriodeFestius[]>();
  ...
  fullYear(data: Date, num:number): boolean{
    ...
    const timeNextYear: number = new Date('01/01/'+nextYear.toString()).getTime(); 
  ...
  esborra(id: number):void{
    this.periodeFestiusService.delete(id).subscribe(res => {if(res.status=='ok'){
      location.reload();
    }});    
